...
<?php
 $tablename="products"; //calls
  $title="Products"; 
$q = $_GET['q'];
 $sql="SELECT * FROM ".$tablename." WHERE  fd1=2 AND fd3 LIKE '%$q%' limit 20";
    ...
    $no++;
           if ($result->num_rows > 0) {
               ...
               if($row["fd1"]==2)
               {
                echo "<td><button type=\"button\" class=\"btn btn-success btn-sm text-white\">Shashwat</button></td>";
               
               }
               else{
                echo "<td><button type=\"button\" class=\"btn btn-danger btn-sm text-white\">Hindtex</button></td>";
