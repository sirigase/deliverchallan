...
if ($tag == "3") {
    ...
    $sql = "SELECT * FROM " . $tablename . " WHERE fd1=$editDCno AND fd2=$editCompany"; // alldcreceipts
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            $total = $row["fd4"]; //total
            $assyr = $row["fd5"];  //assyr
            $purpose = $row["fd6"];  //purpose
            $transport = $row["fd7"];  //transport
            $vehicleno = $row["fd8"]; //vehicle
            $date = $row["fd9"]; //date
            $dcno = $editDCno;
        }
    }
    //retrieve dchallan details starts
    $sql = "SELECT * FROM " . $tablename2 . " WHERE fd10=$editDCno AND fd8=$editCompany  order by id DESC"; //challandetails
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $fd1b = $row["fd1"]; //pat
           ...
           $fd4b = $row["fd4"]; //nos
            $fd5b = $row["fd5"]; //qreq
            $fd6b = $row["fd6"]; //amt
            $fd12b = $row["fd12"]; //amt on that date
            $fd13b = $row["fd13"]; //hsn
            $fd14b = $row["fd14"]; // tax
            $combineAll .= $fd1b . "|" . $fd2b . "|" . $fd5b . "|" . $fd4b . "|" . $fd6b . "|" . $fd12b ."|" . $fd13b ."|" . $fd14b . ";";
        }
    }
...
if ($tag == "2") {
    ...
    $conn = new mysqli($servername, $username, $password, $dbname);
    if ($allFine == 1) {
        // delete previous challan details in db
        $sql = "DELETE FROM " . $tablename2 . " WHERE fd10=$dcno AND fd8=$company";
        if ($conn->query($sql) === TRUE) {
        }
        // delete alldcreceipts before    
        $sql = "DELETE FROM " . $tablename . " WHERE fd1=$dcno AND fd2=$company";
        if ($conn->query($sql) === TRUE) {
        }
    }
  ...
  ('$dcno','$company','$buyer','$total','$assyr','$purpose','$transport','$vehicleno','$date','$fd1d')";
            ...
            header('Location:ViewDcReceipts.php?id='.$dcno.'&cy='.$company.'&s=2');
        }
        if ($delActive == 0) {
...
<script>
  function deleteFormItem(x) {
   ...
   {
     // var lineItems=document.getElementById("formcount").value;
         ...
         <div class="page-wrapper">
                        ...
                        <div class="ms-auto text-end">
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page">Edit DC</li>
                                </ol>
                            </nav>
                        </div>
                    </div>
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                  <div class="col-md-6">
                        <div class="card">
                            <form class="form-horizontal" action="EditDcReceipts.php?s=2" name="addItemsform" id="addItemsform" method="post" onsubmit="return validateFormItem()">
                                <div class="card-body">
                                 <?php
...
$conn->close();
?>
 <input type="text" class="form-control"  value="<?php echo $fd1; ?>" disabled>
...
?>
                                ...
                                <label for=""
                                        ...
                                        <input type="text" class="form-control"
                                                value="<?php echo $dcno; ?>" disabled>
                                        
                                        <input type="hidden" name="dcno" id="dcno" value="<?php echo $dcno; ?>">  
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-9">
                                          <input type="text" class="form-control" name="assyr" value="<?php echo $assyr; ?>">
